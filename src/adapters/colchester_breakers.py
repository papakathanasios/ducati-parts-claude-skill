"""Colchester Motorcycle Breakers adapter (colchesterbreakers.co.uk).

Custom site. 300+ models, 15,000+ parts.
Search via /parts/?s=QUERY.
"""
...
class ColchesterBreakersAdapter(PlaywrightBaseAdapter):
    source_name = "colchester_breakers"
    language = "en"
    country = "GB"
    currency = "GBP"
    ...
    async def _extract_listings(self, page: Page, query: str) -> list[RawListing]:
        ...
        # Try WooCommerce and generic product selectors
        cards = await page.query_selector_all(
            "li.product, .type-product, .product-card, "
    ...
    async def _parse_card(self, card) -> RawListing | None:
        title_el = await card.query_selector(
            ".woocommerce-loop-product__title, h2, h3, .product-title a"
        )
        if not title_el:
            return None
