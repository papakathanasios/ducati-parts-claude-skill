"""Motoye adapter – Spanish motorcycle breaker (motoye.es).

PrestaShop + IqitSearch. Search via /es/buscar?controller=search&s=QUERY.
"""
...
class MotoyeAdapter(PlaywrightBaseAdapter):
    source_name = "motoye"
    language = "es"
    country = "ES"
    currency = "EUR"
    base_url = "https://www.motoye.es"
    ...
    async def _parse_card(self, card) -> RawListing | None:
        title_el = await card.query_selector(
            ".product-title a, h3 a, h5 a, a.product-name, .product-name"
        )
        if not title_el:
            return None
